...
+(CUSFlashLabel*)setCUSFlashLabel:(NSString*)str
{
    CUSFlashLabel *chatLabel;
    
    if (version>=8)
    {
        chatLabel = [[CUSFlashLabel alloc]initWithFrame:CGRectMake(50, -5, 150, 40)];
    }else
    {
        chatLabel = [[CUSFlashLabel alloc]initWithFrame:CGRectMake(50, 0, 150, 40)];
    }
    
    chatLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15];
    chatLabel.text = str;
    
    [chatLabel setTextColor:[UIColor whiteColor]];
    [chatLabel setSpotlightColor:ThemeColor];
    [chatLabel startAnimating];
